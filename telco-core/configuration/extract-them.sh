...
with open("rendered-core-config.yaml", 'r') as f:
    ...
    for doc in docs:
        ...
        if doc.get('kind') == 'Policy':
            ...
            policy_wave = doc.get('metadata', {}).get('annotations', {}).get('ran.openshift.io/ztp-deploy-wave', '0')
            ...
            for pt_idx, pt in enumerate(policy_templates):
                ...
                if obj_def.get('kind') == 'ConfigurationPolicy':
                    ...
                    for ot_idx, ot in enumerate(obj_templates):
                        ...
                        if obj_template:
                            obj_name = obj_template.get('kind')
                            obj_kind = obj_template.get('metadata', {}).get('name')
                            filename = f"{output_dir}/{policy_wave}-{policy_name}-{obj_name}-{obj_kind}-{ot_idx}.yaml"
